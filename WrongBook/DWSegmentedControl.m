...
-(void)setUpUI{
    ...
    CGFloat width = (self.frame.size.width -  40)/self.titles.count;
...
//    self.bgView.clipsToBounds = YES;
    ...
    for (int i = 0;i<self.titles.count; i++) {
        ...
        titleLb.layer.cornerRadius = height / 2;
        titleLb.layer.borderWidth = 1;
        titleLb.layer.borderColor = [COLOR_MAIN CGColor];
        titleLb.textColor = self.normalLabelColor?self.normalLabelColor:[UIColor blackColor];
        titleLb.font = [UIFont systemFontOfSize:14];
        titleLb.frame = CGRectMake(20+width*i+10, 10, width-20, height);
    ...
    self.bgView.frame = CGRectMake(20, 10, width, height);
    _bgView.backgroundColor = self.selectedViewColor?self.selectedViewColor:[UIColor redColor];
   ...
   UIView *topLabelView = [[UIView alloc]initWithFrame:self.bounds];
    ...
    for (int i = 0;i<self.titles.count; i++) {
        ...
        titleLb.font = [UIFont systemFontOfSize:14];
        titleLb.frame = CGRectMake(20+width*i+10, 0, width-20, height);
        titleLb.textAlignment = NSTextAlignmentCenter;
...
-(void)selectedLabel:(UITapGestureRecognizer *)sender{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.bgView.left = (sender.view.tag - 1000)*width+20;
...
- (void)sclectedIndex:(NSInteger)index{
    CGFloat width = (self.frame.size.width - 40)/self.titles.count;
    [UIView animateWithDuration:0.3 animations:^{
        self.bgView.left = index * width+20;
        self.frontLabelbgView.left = -index * width;
        [self.bgView layoutIfNeeded];
    }];
